...
transaction_signals = DatabaseSignals()
del DatabaseSignals
...
def send_robust_and_log_errors(signal_name, **kwargs):
    signal = getattr(transaction_signals, signal_name)
...
def enter_transaction_management(managed=True, using=None):
    transaction_signals.pre_transaction_management.send(sender=conn(using))
    yield
...
def leave_transaction_management(using=None):
    ...
    if is_dirty and not IS_DJANGO_12:
        transaction_signals.pre_rollback.send(sender=connection)
    try:
        yield
    finally:
        if is_dirty and not IS_DJANGO_12:
            send_robust_and_log_errors("post_rollback", sender=connection)
        send_robust_and_log_errors(
            "post_transaction_management", sender=connection)
